...
const total=normalized.reduce((s,i)=>s+i.subtotal,0),orderNumber=`HP-${Date.now().toString().slice(-8)}`;
const{data:order,error:oe}=await supabase.from("orders").insert({order_number:orderNumber,channel:"ONLINE",customer_name,phone:phone||null,line_id:line_id||null,address:address||null,total,status:"NEW",note:note||null}).select("id,order_number,created_at").single();if(oe)throw oe;
...
if(process.env.TELEGRAM_BOT_TOKEN&&process.env.TELEGRAM_CHAT_ID)try{await fetch(`https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/sendMessage`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({chat_id:process.env.TELEGRAM_CHAT_ID,text:`🪄 NEW ORDER ${orderNumber}\nลูกค้า: ${customer_name}\nติดต่อ: ${[phone,line_id].filter(Boolean).join(" / ")||"-"}\nรายการ: ${itemText}\nยอดรวม: ฿${total.toFixed(2)}\nหมายเหตุ: ${note||"-"}`})})}catch(e){console.error("Telegram failed",e)}
